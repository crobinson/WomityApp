...
-(void)splitViewController:(UISplitViewController *)svc willHideViewController:(UIViewController *)aViewController withBarButtonItem:(UIBarButtonItem *)barButtonItem forPopoverController:(UIPopoverController *)pc
{
    ...
    self.masterBarButtonItem.title = NSLocalizedString(@"Master", @"Master");
...
-(void)callNewViewControllerByString:(NSString *)string{
    ...
    if(detailControler != self.currentDetailController)
    {
        ...
        if (self.currentDetailController)
        {
            
        }
        
        // update controllers in splitview
        UIViewController* navegation = [self.splitViewController.viewControllers objectAtIndex:0];
        self.splitViewController.viewControllers = [NSArray arrayWithObjects:navegation,detailRootController, nil];
...
/*-(void)callDetailViewControllerByString:(NSString *)string withNoticia:(Noticia *)noticia andArray:(NSMutableArray *)array{
    ...
    UINavigationController* detailRootController = [storyboard instantiateViewControllerWithIdentifier:string];
    iPadDetailViewController* detailControler = (iPadDetailViewController *)detailRootController.topViewController;
    detailControler.noticia = noticia;
    detailControler.arrayFromNews = array;
    detailControler.frommenu = YES;
