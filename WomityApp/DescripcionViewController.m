...
@synthesize datawomit, myView, myscrollView, responseData, idWomity, aSesion, vistareloj, reloj, botonVotar, comentar, campocomentario, vistaimagen, titulo, relojimagen;
...
- (IBAction)eliminaropcion:(id)sender{
    ...
    alertOpcion = [[UIAlertView alloc] initWithTitle:@"WOMITY" message:@"¿Estás seguro que deseas eliminar esta opción?" delegate:self cancelButtonTitle:@"NO" otherButtonTitles:@"SI", nil];
    [alertOpcion show];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    } else if (alertBorrador == alertView){
        

        
        
        NSString *post = [NSString stringWithFormat:@"&idSession=%@&idWomity=%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"id"], [NSString stringWithFormat:@"%i", borradorTag]];
    ...
    }else if (alertFinalizado == alertView){
        NSString *post = [NSString stringWithFormat:@"&idSession=%@&idWomity=%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"id"], [NSString stringWithFormat:@"%i", finalizadoTag]];
...
-(void)eliminarFinalizar:(id) sender{
    UIButton *temporal = (UIButton *) sender;
    finalizadoTag = temporal.tag;
    alertFinalizado  = [[UIAlertView alloc] initWithTitle:@"WOMITY" message:@"Estas seguro de querer eliminar este Womit finalizado?" delegate:self cancelButtonTitle:@"NO" otherButtonTitles:@"SI", nil];
...
-(void)eliminarBorrador:(id) sender{
    ...
    UIButton * temporal = (UIButton *)sender;
    borradorTag = temporal.tag;
    alertBorrador = [[UIAlertView alloc] initWithTitle:@"WOMITY" message:@"Estas seguro de querer eliminar este Borrador?" delegate:self cancelButtonTitle:@"NO" otherButtonTitles:@"SI", nil];
    [alertBorrador show];
}
...
- (void) viewDidAppear:(BOOL)animated
{
 //   [super viewDidLoad];
    ...
    NSLog(@"%@",datawomit);
        ...
        if([[[NSUserDefaults standardUserDefaults] valueForKey:@"tipowomit"] isEqualToString:@"F"])
            titulo.text = @"womits finalizados";
    botonVotar = [UIButton buttonWithType:UIButtonTypeCustom];
    botonVotar.frame = CGRectMake(239, 10, 66, 47);
        tipo = [[NSUserDefaults standardUserDefaults] valueForKey:@"tipowomit"];
    
        if([tipo isEqualToString:@"A"]){
    botonVotar.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"Votar"]];
    [botonVotar addTarget:self action:@selector(gotovotar:) forControlEvents:UIControlEventTouchUpInside];
        }
    if([tipo isEqualToString:@"B"]){
        botonVotar.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"bcaBorrador"]];
        [botonVotar addTarget:self action:@selector(eliminarBorrador:) forControlEvents:UIControlEventTouchUpInside];
        botonVotar.tag = [[datawomit objectForKey:@"idWomity"] integerValue];
        //NSLog(@"%i", botonVotar.tag);
    }
        
        if([tipo isEqualToString:@"F"]){
            botonVotar.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"bcaFinalizar"]];
            [botonVotar addTarget:self action:@selector(eliminarFinalizar:) forControlEvents:UIControlEventTouchUpInside];
            botonVotar.tag = [[datawomit objectForKey:@"idWomity"] integerValue];
            //NSLog(@"%i", botonVotar.tag);
        }
    ...
    [myscrollView addSubview:titulo];
        ...
        [myscrollView addSubview:botonVotar];
    ...
    if([tipo isEqualToString:@"A"]){
        ...
        UIImageView *botonesimg = [[UIImageView alloc] initWithFrame:CGRectMake(vistagris.frame.origin.x, vistagris.frame.origin.y + vistagris.frame.size.height + 10, 300, 29)];
        if(![tipo isEqualToString:@"F"]){
    if([[datawomit objectForKey:@"PermisoOpciones"]isEqualToString:@"S"] || [[datawomit objectForKey:@"soyCreador"]isEqualToString:@"TRUE"]){
    
    UIImageView *botonesimg = [[UIImageView alloc] initWithFrame:CGRectMake(vistagris.frame.origin.x, vistagris.frame.origin.y + vistagris.frame.size.height + 10, 300, 29)];
    botonesimg.image = [UIImage imageNamed:@"tabs"];
        
    
    [myscrollView addSubview:botonesimg];
    
    UILabel *anadir = [[UILabel alloc] initWithFrame:CGRectMake(vistagris.frame.origin.x + 12, botonesimg.frame.origin.y + 10, 100, 10)];
    anadir.text = @"Añadir opción";
    anadir.backgroundColor = [UIColor clearColor];
    anadir.textColor = [UIColor whiteColor];
    [anadir setFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:11]];
    [myscrollView addSubview:anadir];
    
    UIButton *boton1 = [UIButton buttonWithType:UIButtonTypeCustom];
    boton1.frame = CGRectMake(botonesimg.frame.origin.x, botonesimg.frame.origin.y, 100, 29);
    [boton1 addTarget:self action:@selector(gotocreaopcion:) forControlEvents:UIControlEventTouchUpInside];
    [myscrollView addSubview:boton1];
    
    UILabel *invitar = [[UILabel alloc] initWithFrame:CGRectMake(vistagris.frame.origin.x + 134, botonesimg.frame.origin.y + 10, 100, 10)];
    invitar.text = @"Invitar";
    invitar.backgroundColor = [UIColor clearColor];
    invitar.textColor = [UIColor whiteColor];
    [invitar setFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:11]];
    [myscrollView addSubview:invitar];
    
    UIButton *boton2 = [UIButton buttonWithType:UIButtonTypeCustom];
    boton2.frame = CGRectMake(botonesimg.frame.origin.x + boton1.frame.size.width, botonesimg.frame.origin.y, 100, 29);
    [boton2 addTarget:self action:@selector(gotoinvitar:) forControlEvents:UIControlEventTouchUpInside];

    [myscrollView addSubview:boton2];
    
    UILabel *modificar = [[UILabel alloc] initWithFrame:CGRectMake(vistagris.frame.origin.x + 204, botonesimg.frame.origin.y + 10, 100, 10)];
    modificar.text = @"Modificar tiempo";
    modificar.backgroundColor = [UIColor clearColor];
    modificar.textColor = [UIColor whiteColor];
    [modificar setFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:11]];
    [myscrollView addSubview:modificar];
    
    UIButton *boton3 = [UIButton buttonWithType:UIButtonTypeCustom];
    boton3.frame = CGRectMake(boton2.frame.origin.x + boton2.frame.size.width, botonesimg.frame.origin.y, 100, 29);
    [boton3 addTarget:self action:@selector(gotomodificar:) forControlEvents:UIControlEventTouchUpInside];
    [myscrollView addSubview:boton3];
    
    }else{
        botonesimg.frame = CGRectMake(vistagris.frame.origin.x, vistagris.frame.origin.y + vistagris.frame.size.height + 10, 300, 0);
    }
    ...
    for (int i=0; i<opciones.count; i++) {
        ...
        */
        ...
        UIScrollView *miniscroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, vistablanca.frame.size.width, vistablanca.frame.size.height)];
        miniscroll.backgroundColor = [UIColor clearColor];
        miniscroll.pagingEnabled = YES;
        miniscroll.bounces = NO;
        miniscroll.showsHorizontalScrollIndicator = NO;
        
        [vistablanca addSubview:miniscroll];
        
        
        
        
        UIView *vistatools = [[UIView alloc] initWithFrame:CGRectMake(0, 0, vistablanca.frame.size.width, vistablanca.frame.size.height)];
        vistatools.backgroundColor = [UIColor colorWithRed:(229/255.0) green:(229/255.0) blue:(229/255.0) alpha:1];
        [miniscroll addSubview:vistatools];
        
        UIButton *eliminar = [UIButton buttonWithType:UIButtonTypeCustom];
        eliminar.frame = CGRectMake(160, vistablanca.frame.size.height/2 - 19, 36, 38);
        [eliminar addTarget:self action:@selector(eliminaropcion:) forControlEvents:UIControlEventTouchUpInside];
        eliminar.tag = i;
        eliminar.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"iconoEliminar"]];
        
        [vistatools addSubview:eliminar];
        
        UIButton *modificar = [UIButton buttonWithType:UIButtonTypeCustom];
        modificar.frame = CGRectMake(100, vistablanca.frame.size.height/2 - 14, 42, 33);
        [modificar addTarget:self action:@selector(gotomodificaropcion:) forControlEvents:UIControlEventTouchUpInside];
        modificar.tag = i;
        modificar.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"iconoModificar"]];
        
        [vistatools addSubview:modificar];
        
        miniscroll.contentSize = CGSizeMake(vistablanca.frame.size.width*2, vistablanca.frame.size.height);
        miniscroll.delegate = self;
        [miniscroll scrollRectToVisible:CGRectMake(vistablanca.frame.size.width,0,vistablanca.frame.size.width,vistablanca.frame.size.height) animated:NO];
        gotoimg.frame = CGRectMake(thumbopcion.frame.origin.x + vistablanca.frame.size.width, 12, 45, 45);
        [gotoimg addTarget:self action:@selector(verimagen:) forControlEvents:UIControlEventTouchUpInside];
        gotoimg.tag = i;
        
        UIButton *gotoweb = [UIButton buttonWithType:UIButtonTypeCustom];
        gotoweb.frame = CGRectMake(web.frame.origin.x + vistablanca.frame.size.width, web.frame.origin.y, web.frame.size.width, web.frame.size.height);
        [gotoweb addTarget:self action:@selector(gotourlweb:) forControlEvents:UIControlEventTouchUpInside];
        gotoweb.tag = i;
        
        [miniscroll addSubview:gotoimg];
        [miniscroll addSubview:gotoweb];
        
        if([[datawomit objectForKey:@"PermisoOpciones"]isEqualToString:@"N"] && ![[datawomit objectForKey:@"soyCreador"]isEqualToString:@"TRUE"])
            miniscroll.scrollEnabled = NO;
...
if(![tipo isEqualToString:@"B"]){
    ...
    //UIButtonTypeCustom
    UIButton *gotoparticipantes = [UIButton buttonWithType:UIButtonTypeCustom];
    gotoparticipantes.frame = mensaje1.frame;
    [gotoparticipantes addTarget:self action:@selector(gotoparticipantes:) forControlEvents:UIControlEventTouchUpInside];
    
    
    if(![tipo isEqualToString:@"F"]){
    [myscrollView addSubview:mensaje1];
    [myscrollView addSubview:gotoparticipantes];
    }
...
{
    if(self.view.frame.size.width == 320){
         [myscrollView removeGestureRecognizer:tapScroll];
        [UIView beginAnimations:nil context:NULL];
        [UIView setAnimationDuration:0.25];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    [alerta show];
    [reloj stopAnimating];
    [vistareloj setFrame:CGRectMake(0, 800, 320, 440)];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if([urlconexion isEqualToString:@"http://www.womity.com/ws/agregarComentario"] || [urlconexion isEqualToString:@"http://www.womity.com/ws/EliminarOpcionWomity"] || [urlconexion isEqualToString:@"http://www.womity.com/ws/eliminarComentario"])
    {
        ...
        if([[json objectForKey:@"strMensaje"] isEqualToString:@"OpcionEliminada"])
            alerta = [[UIAlertView alloc] initWithTitle:@"Mensaje" message:@"Opción Eliminada" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        
        if([[json objectForKey:@"strMensaje"] isEqualToString:@"ComentarioEliminado"])
            alerta = [[UIAlertView alloc] initWithTitle:@"Mensaje" message:@"Comentario Eliminado" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        if([[json objectForKey:@"strMensaje"] isEqualToString:@"OK"] && [urlconexion isEqualToString:@"http://www.womity.com/ws/agregarComentario"])
            alerta = [[UIAlertView alloc] initWithTitle:@"Mensaje" message:@"Comentario enviado" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        
        if([[json objectForKey:@"strMensaje"] isEqualToString:@"WomitNoPerteneceAUsuario"] && [urlconexion isEqualToString:@"http://www.womity.com/ws/EliminarOpcionWomity"])
            alerta = [[UIAlertView alloc] initWithTitle:@"Mensaje" message:@"No tienes permiso para eliminar la opción" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        
        if([[json objectForKey:@"strMensaje"] isEqualToString:@"WomitNoPerteneceAUsuario"] && [urlconexion isEqualToString:@"http://www.womity.com/ws/eliminarComentario"])
            alerta = [[UIAlertView alloc] initWithTitle:@"Mensaje" message:@"No tienes permiso para eliminar el comentario" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        ...
        {
            UIAlertView *alerta = [[UIAlertView alloc] initWithTitle:@"Mensaje" message:@"Womit eliminado correctamente" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alerta show];
    ...
    if([[[NSUserDefaults standardUserDefaults] valueForKey:@"notificaciones"] isEqualToString:@"true"]){
        myscrollView.contentOffset = CGPointMake(0, myscrollView.frame.size.height);
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"participantes"]){
        reloadPage = YES;
        ...
        controller.datawomit = [datawomit mutableCopy];
    }
...
- (IBAction)gotoparticipantes:(id)sender{    
    
    [self performSegueWithIdentifier:@"participantes" sender:self];
}
